...
最近复验：2026-09-25。源码和 TEST_REPORT.md 中最新四轮维护结果是事实依据。

## 2026-09-25 维护增量

- 路由事件区分普通片段与页面切换，文章异步载入后恢复有效章节；目录保留修饰键点击，并正确编码自定义标题 ID。
- BlogList 同时监听 popstate 和 app:navigate，点击导航回到无筛选列表时，输入、标签与 URL 一致。
- 文章 id/slug 使用同一标识空间检查，含草稿；同篇可相等，跨文章冲突构建前即报错并列出文件。
- Markdown 目录避开正文已有 ID 与页面保留 ID（root、main-content、page-structured-data），保留有效显式标题锚点。构建片段检查支持 URL 编码。
- GitHub Hook 的 enabled 开关由首页、项目页或搜索弹窗决定；纯阅读不发请求，停用取消请求，再启用复用有效缓存，手动刷新只对本次请求绕过缓存。
- 全量 73/73 行为测试、8 页构建、16/16 Chromium 浏览器验收通过；覆盖率门禁通过，主入口 gzip 68,093 bytes。最新覆盖率报告改到 test-results/coverage/。
- 本轮独立复核发现的页面容器 ID 碰撞已修复并回归。浏览器截图已检查；服务已关闭。提交状态以 Git 历史为准，线上发布状态以 GitHub Actions 部署结果为准。

下方为此前架构说明与历史交接；涉及旧数量和部署状态时，以本节及最新 TEST_REPORT.md 为准。
